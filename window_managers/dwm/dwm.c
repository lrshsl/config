...
Atom getatomprop(Client *c, Atom prop) {
	...
	if ( XGetWindowProperty(
			 dpy, c->win, prop, 0L, sizeof atom, False, XA_ATOM, &da, &format,
			 &nitems, &dl, &p
		 ) == Success &&
	     p ) {
...
int gettextprop(Window w, Atom atom, char *text, unsigned int size) {
	...
	if ( name.encoding == XA_STRING ) {
		...
		XmbTextPropertyToTextList(dpy, &name, &list, &n) >= Success && n > 0 &&
		*list
	) {
...
void grabbuttons(Client *c, int focused) {
	updatenumlockmask();
	{
		...
		if ( !focused ) {
			XGrabButton(
				dpy, AnyButton, AnyModifier, c->win, False, BUTTONMASK,
		...
		for ( i = 0; i < LENGTH(buttons); i++ ) {
			if ( buttons[i].click == ClkClientWin ) {
				for ( j = 0; j < LENGTH(modifiers); j++ ) {
					XGrabButton(
						dpy, buttons[i].button, buttons[i].mask | modifiers[j],
		...
		for ( k = start; k <= end; k++ ) {
			for ( i = 0; i < LENGTH(keys); i++ ) {
				...
				if ( keys[i].keysym == syms[(k - start) * skip] ) {
					for ( j = 0; j < LENGTH(modifiers); j++ ) {
						XGrabKey(
							dpy, k, keys[i].mod | modifiers[j], root, True,
	...
	do {
		...
		case MotionNotify:
			...
			if ( abs(selmon->wx - nx) < snap ) {
				...
				abs((selmon->wx + selmon->ww) - (nx + WIDTH(c))) < snap
			) {
			...
			if ( abs(selmon->wy - ny) < snap ) {
				...
				abs((selmon->wy + selmon->wh) - (ny + HEIGHT(c))) < snap
			) {
...
void updateclientlist(void) {
	...
	for ( m = mons; m; m = m->next ) {
		for ( c = m->clients; c; c = c->next ) {
			XChangeProperty(
				dpy, root, netatom[NetClientList], XA_WINDOW, 32,
